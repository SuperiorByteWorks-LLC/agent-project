...
tags: [science, writing, markdown, mermaid, diagrams, documentation, style-guide]
load_frequency: high
license: Apache-2.0
metadata:
  ...
  imported-from: https://github.com/K-Dense-AI/claude-scientific-skills
...
Based on real-world usage at scale (1,000+ skills), here are the key lessons for creating readable, maintainable diagrams:

### Layout Direction

**Use `flowchart LR` (Left-to-Right) by default** — vertical diagrams (TB) become unreadable on narrow screens. LR diagrams fit GitHub's rendering width better.
...
**Group related nodes in subgraphs** — this creates visual hierarchy and makes complex diagrams scannable.
...
**Keep node labels short** — use abbreviations or short names. Long labels break layout.

| ❌ Bad | ✅ Good |
|--------|---------|
| `long-descriptive-node-name` | `LDNN` or short desc |
| `comprehensive-markdown-and-mermaid-writing-skill` | `MMW` or `mermaid-skill` |

### Avoid Wide Diagrams

**If a diagram is >8 nodes wide, restructure** — use subgraphs, or switch to a different diagram type:

- **Flowchart**: 6-8 nodes max per row
- **User Journey**: For step-by-step processes
- **Git Graph**: For version/branch history
- **Requirement Diagram**: For dependencies
...
This renders cleanly in GitHub without horizontal scrolling.
